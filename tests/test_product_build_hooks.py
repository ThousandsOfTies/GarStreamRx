...
class ProductBuildHookTests(unittest.TestCase):
    def _run(self, script: Path, environment: dict[str, str] | None = None) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            ["bash", str(script)], cwd=script.parents[1], text=True, capture_output=True,
    ...
    def test_target_manifest_never_deploys_untracked_environment_file(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            (root / "scripts").mkdir()
            shutil.copy2(REPOSITORY_ROOT / "scripts" / "product-target-build.sh", root / "scripts")
            ...
            (root / "config/gar-stream-rx.target.env").write_text("GAR_STREAM_DISCOVERY_PEERS=192.0.2.10\n", encoding="utf-8")
            builder = root / "fake-builder.sh"
            builder.write_text("#!/bin/sh\nprintf binary > \"$1\"\n", encoding="utf-8")
            builder.chmod(0o755)

            result = self._run(root / "scripts/product-target-build.sh", {"GAR_RX_TARGET_BUILDER": str(builder)})
            ...
            self.assertFalse((root / "artifacts/from-codespace/files/gar-stream-rx/gar-stream-rx.env").exists())
